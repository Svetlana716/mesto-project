export default class FormValidator {
  constructor({ selectorsAndClasses }, //selectorsAndClasses - объект настроек
                form) {//form - элемент той формы, которая валидируется
  ...
  _showInputError(input, errorMessage) {// показывает элемент ошибки
  ...
  _hideInputError(input) {//скрывает элемент ошибки
    const formError = this._form.querySelector(`.${input.id}-error`);
  ...
  _isValid(input) {//проверяет валидность поля и  внутри вызывает приватные методы _showInputError или _hideInputError
    ...
    if (!input.validity.valid) {//в условии указал вместо функций приватные методы this._showInputError и this._hideInputError
  ...
  _hasInvalidInput() { // проверяем валидность всех полей ввода формы
    return this.inputList.some((inputElement) => {
      return !inputElement.validity.valid;
    });
  }

  _toggleButtonState() {// метод условие переключение кнопки сабмит
    if (this._hasInvalidInput()) {
      this._button.classList.add(this._selectorsAndClasses.inactiveButtonClass);
      this._button.setAttribute("disabled", true);
    } else {
      this._button.classList.remove(this._selectorsAndClasses.inactiveButtonClass);
